...
public class LessonController {
    ...
    @PostMapping("/save") // http://localhost:8081/lessons/save + POST + JSON
    ...
    @PreAuthorize("hasAnyAuthority('ADMIN','MANAGER','ASSISTANT_MANAGER')")
    @DeleteMapping("/delete/{id}") // http://localhost:8081/lessons/delete/2 + DELETE
    public ResponseMessage deleteLesson(@PathVariable Long id){
        return lessonService.deleteLessonById(id);
    }

    // Not:  ODEV : getAllWithPage ************************************
    @GetMapping("/findLessonByPage") // http://localhost:8081/lessons/findLessonByPage?page=0&size=10&sort=lessonName&type=desc
    @PreAuthorize("hasAnyAuthority('ADMIN','MANAGER','ASSISTANT_MANAGER')")
    ...
    @GetMapping("/getLessonByName") // http://localhost:8081/lessons/getLessonByName?lessonName=java
    @PreAuthorize("hasAnyAuthority('ADMIN','MANAGER','ASSISTANT_MANAGER')")
    ...
    @PreAuthorize("hasAnyAuthority('ADMIN','MANAGER','ASSISTANT_MANAGER')")
    @PutMapping("/update/{lessonId}") //http://localhost:8081/lessons/update/1
